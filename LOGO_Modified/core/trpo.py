...
def trpo_step(policy_net, value_net_1, value_net_2, states, actions, returns, advantages, max_kl, damping, l2_reg, epsilon, fixed_log_probs=None,use_fim=True):

    """update critic"""

    if value_net_2:
        def get_value_loss_1(flat_params):
            set_flat_params_to(value_net_1, tensor(flat_params))
            for param in value_net_1.parameters():
                if param.grad is not None:
                    param.grad.data.fill_(0)
            values_pred = value_net_1(states)
            ...
            value_loss.backward()
            return value_loss.item(), get_flat_grad_from(value_net_1.parameters()).cpu().numpy()

        def get_value_loss_2(flat_params):
            set_flat_params_to(value_net_2, tensor(flat_params))
            for param in value_net_2.parameters():
                if param.grad is not None:
                    param.grad.data.fill_(0)
            values_pred = value_net_2(states)
            value_loss = (values_pred - returns).pow(2).mean()
            # weight decay
            for param in value_net_2.parameters():
                value_loss += param.pow(2).sum() * l2_reg
            value_loss.backward()
            return value_loss.item(), get_flat_grad_from(value_net_2.parameters()).cpu().numpy()

        flat_params, _, opt_info = scipy.optimize.fmin_l_bfgs_b(get_value_loss_1,
                                                                get_flat_params_from(value_net_1).detach().cpu().numpy(),
                                                                maxiter=25)
        set_flat_params_to(value_net_1, tensor(flat_params))

        flat_params, _, opt_info = scipy.optimize.fmin_l_bfgs_b(get_value_loss_2,
                                                                get_flat_params_from(value_net_2).detach().cpu().numpy(),
                                                                maxiter=25)
        set_flat_params_to(value_net_2, tensor(flat_params))
