...
parser.add_argument('--delta', type=float, default=0.95, metavar='G',
					help='KL decay')
parser.add_argument('--use_ddqn', dest='use_ddqn', action='store_true')
...
value_net_1 = Value(state_dim,hidden_size=nn_size)
value_net_exp_1 = Value(state_dim,hidden_size=nn_size)

if args.use_ddqn:
	value_net_2 = Value(state_dim,hidden_size=nn_size)
	value_net_exp_2 = Value(state_dim,hidden_size=nn_size)
else:
	value_net_2 = None
	value_net_exp_2 = None
...
if args.use_ddqn:
	to_device(device, policy_net, value_net_1, value_net_exp_1, value_net_2, value_net_exp_2, discrim_net, discrim_criterion)
else:
	to_device(device, policy_net, value_net_1, value_net_exp_1, discrim_net, discrim_criterion)
...
def update_params(batch, i_iter,kl):
	...
	with torch.no_grad():
		values = value_net_1(states)
		values_exp = value_net_exp_1(states)
		if args.use_ddqn:
			values_2 = value_net_2(states)
			values_exp_2 = value_net_exp_2(states)

			values = torch.min(values, values_2)
			values_exp = torch.min(values_exp, values_exp_2)
	...
	trpo_step(policy_net, value_net_1, value_net_2, states, actions, returns, advantages, args.max_kl, args.damping, args.l2_reg, args.clip_epsilon)
	if (kl > 6e-7):
		trpo_step(policy_net, value_net_exp_1, value_net_exp_2, states, actions, returns_exp, advantages_exp, kl, args.damping, args.l2_reg, args.clip_epsilon, fixed_log_probs = fixed_log_probs)
